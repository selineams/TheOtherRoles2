...
namespace TheOtherRoles {
    public class CustomOption {
        ...
        public static CustomOption Create(int id, CustomOptionType type, string name, bool defaultValue, CustomOption parent = null, bool isHeader = false, Action onChange = null, string heading = "") {
            return new CustomOption(id, type, name, new string[]{ "<color=#CCCCCCFF>关</color>", "<color=#FFFF00FF>开</color>" }, defaultValue ? "<color=#FFFF00FF>开</color>" : "<color=#CCCCCCFF>关</color>", parent, isHeader, onChange, heading);
        }
        ...
        public static int pasteFromClipboard() {
            ...
            try {
                ...
                ShareOptionSelections();
                if (TheOtherRolesPlugin.Version > versionInfo && versionInfo < TheOtherRolesPlugin.Version) {
                    vanillaOptionsFine = false;
                    FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(PlayerControl.LocalPlayer, "房主消息: 粘贴原版设置失败，模组设置已应用!");
                } else {
                ...
                string errorStr = allSettings.Length > 2 ? allSettings.Substring(0, 3) : "(空的粘贴板) ";
                FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(PlayerControl.LocalPlayer, $"房主信息: 您试图粘贴无效的设置: \"{errorStr}...\"");
    ...
    class LobbyViewSettingsPatch{
                ...
                if (option.isHeader && (int)optionType != 99 || (int)optionType == 99 && curType != option.type) {
                    ...
                    if ((int)optionType == 99)
                        categoryHeaderMasked.Title.text = new Dictionary<CustomOptionType, string>() { { CustomOptionType.Impostor, "内鬼职业" }, { CustomOptionType.Neutral, "中立职业" },
                            { CustomOptionType.Crewmate, "船员职业" }, { CustomOptionType.Modifier, "附加效果" } }[curType];
        ...
        public static void createSettingTabs(LobbyViewSettingsPane __instance) {
            ...
            if (TORMapOptions.gameMode == CustomGamemodes.Guesser || TORMapOptions.gameMode == CustomGamemodes.Classic) {
                ...
                // create TOR settings
                createCustomButton(__instance, next++, "TORSettings", "模组设置", CustomOptionType.General);
                   // create TOR settings
                createCustomButton(__instance, next++, "RoleOverview", "职业总览", (CustomOptionType)99);
                // IMp
                createCustomButton(__instance, next++, "ImpostorSettings", "内鬼职业", CustomOptionType.Impostor);

                // Neutral
                createCustomButton(__instance, next++, "NeutralSettings", "中立职业", CustomOptionType.Neutral);
                // Crew
                createCustomButton(__instance, next++, "CrewmateSettings", "船员职业", CustomOptionType.Crewmate);
                // Modifier
                createCustomButton(__instance, next++, "ModifierSettings", "附加效果", CustomOptionType.Modifier);

            } else if (TORMapOptions.gameMode == CustomGamemodes.HideNSeek) {
                // create Main HNS settings
                createCustomButton(__instance, next++, "HideNSeekMain", "躲猫猫设置", CustomOptionType.HideNSeekMain);
                // create HNS Role settings
                createCustomButton(__instance, next++, "HideNSeekRoles", "躲猫猫职业", CustomOptionType.HideNSeekRoles);
    ...
    class GameOptionsMenuStartPatch {
        ...
        private static void createSettingTabs(GameSettingMenu __instance) {
            ...
            if (TORMapOptions.gameMode == CustomGamemodes.Guesser || TORMapOptions.gameMode == CustomGamemodes.Classic) {
                ...
                // create TOR settings
                createCustomButton(__instance, next++, "TORSettings", "模组设置");
                ...
                // IMp
                createCustomButton(__instance, next++, "ImpostorSettings", "内鬼职业");
                ...
                // Modifier
                createCustomButton(__instance, next++, "ModifierSettings", "附加效果");
                createGameOptionsMenu(__instance, CustomOptionType.Modifier, "ModifierSettings");

            } else if (TORMapOptions.gameMode == CustomGamemodes.HideNSeek) {
                // create Main HNS settings
                createCustomButton(__instance, next++, "HideNSeekMain", "躲猫猫");
                createGameOptionsMenu(__instance, CustomOptionType.HideNSeekMain, "HideNSeekMain");
                // create HNS Role settings
                createCustomButton(__instance, next++, "HideNSeekRoles", "躲猫猫职业");
    ...
    class GameOptionsDataPatch
    {
            ...
            foreach (var option in options) {
                if (option.parent == null) {
                    ...
                    if (option.id == 103) //Deputy
                        ...
                        sb.AppendLine($"- {Helpers.cs(Sidekick.color, "随从")}: {option.selections[option.selection].ToString()}");
                    else if (option.id == 358) //Prosecutor
        ...
        public static string buildAllOptions(string vanillaSettings = "", bool hideExtras = false) {
            ...
            if (TORMapOptions.gameMode == CustomGamemodes.HideNSeek) {
                ...
                switch (counter) {
                    case 0:
                        hudString += (!hideExtras ? "" : "第1页: 原版设置 \n\n") + vanillaSettings;
                        break;
                    case 1:
                        hudString += "第2页: 超多职业PLUS设置 \n" + buildOptionsOfType(CustomOption.CustomOptionType.General, false);
                        break;
                    case 2:
                        hudString += "第3页: 职业和附加效果概率 \n" + buildRoleOptions();
                        break;
                    case 3:
    ...
    public class AddToKillDistanceSetting
    {
        ...
        public static void addKillDistance()
        {
            ...
            GameOptionsData.KillDistanceStrings = new(new string[] { "非常短", "短", "中", "长" });
        }
    }
    ...
    public class HudManagerUpdate {
        ...
        public static void Prefix2(HudManager __instance) {
            ...
            for (int i = 0; i < blocks.Length; i++)
            {
                ...
                curBlock = blocks[i];
                if (Helpers.lineCount(curBlock) + Helpers.lineCount(curString) < 42)
                { // original: 43
                    curString += curBlock + "\n\n";
                }
                else
